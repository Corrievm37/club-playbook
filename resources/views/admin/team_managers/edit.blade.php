<x-app-layout>
    <div class="container mx-auto p-6 max-w-2xl">
        <h1 class="text-2xl font-semibold mb-4">Edit Team Manager</h1>
        <form method="POST" action="{{ route('admin.team_managers.update', $membership) }}" class="space-y-6">
            @csrf
            @method('PATCH')
            <div>
            ...
            <div>
                ...
                <select name="managed_age_group" class="mt-1 w-full border rounded p-2" required>
                    @foreach(['U6','U7','U8','U9','U10','U11','U12','U13','U14','U15','U16','U17','U18','U19'] as $ag)
                        <option value="{{ $ag }}" {{ old('managed_age_group', $membership->managed_age_group) === $ag ? 'selected' : '' }}>{{ $ag }}</option>
                    @endforeach
                </select>
