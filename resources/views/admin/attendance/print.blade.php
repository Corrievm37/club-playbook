...
<head>
    ...
    <style>
        ...
        .print-actions a, .print-actions button { display: inline-block; padding: 6px 10px; font-size: 12px; border: 1px solid #111; background: #fff; cursor: pointer; }
        @media print {
...
<h1>{{ ucfirst($session->type) }} — {{ $session->age_group }} — {{ $session->scheduled_at->format('Y-m-d H:i') }}</h1>
<div class="meta">
...
@if(!empty($grouped['unassigned']))
    ...
    <table>
        ...
        <tbody>
        @foreach($grouped['unassigned'] as $a)
            <tr>
                <td>{{ $a->jersey_number ?? '—' }}</td>
                <td>{{ $a->player->last_name }}, {{ $a->player->first_name }}</td>
                <td>{{ $a->player->school_name ?? '—' }}</td>
                <td>
                    @php
                        ...
                        $age = $dob ? $dob->diffInYears($session->scheduled_at) : null;
                    @endphp
                    {{ $dob ? $dob->format('d/m/y') : '—' }}{{ $age !== null ? ' ('.$age.')' : '' }}
                </td>
            </tr>
